...
final class LogInfo {
	 ...
	 * @param categoryTerms Sous cat�gorie
	...
	public void linkStartLogInfo(final LogInfo logInfo) {
		...
		Assertion.checkArgument(logInfo.getLogPattern().isStartLog(), "Ce LogInfo n''est pas un log de d�but : {0}", logInfo);
		Assertion.checkArgument(logInfo.getType().equals(type), "Ce LogInfo n''est pas du m�me type : {0} != {1}", type, logInfo);
		Assertion.checkArgument(logInfo.getCategoryTerms().equals(categoryTerms), "Ce LogInfo n''est pas du m�me sous-type : {0} != {1}", categoryTerms, logInfo);
		//---------------------------------------------------------------------
