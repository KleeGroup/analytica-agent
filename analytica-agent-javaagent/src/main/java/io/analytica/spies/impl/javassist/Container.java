...
public final class Container {
	...
	public static void initCollector(final AnalyticaSpyConf analyticaSpyConf) {
		...
		} else if ("Stack".equals(collectorName)) {
			processConnector = new JmxStackProcessConnector();
		} else {
		...
		PROCESS_CONNECTOR_INSTANCE = processConnector;

		final StringBuilder locations = new StringBuilder();
		for (final String location : analyticaSpyConf.getSystemLocation()) {
			locations.append("/").append(location);
		}

		PROCESS_COLLECTOR_INSTANCE = new KProcessCollector(analyticaSpyConf.getSystemName(), locations.toString(), processConnector);
