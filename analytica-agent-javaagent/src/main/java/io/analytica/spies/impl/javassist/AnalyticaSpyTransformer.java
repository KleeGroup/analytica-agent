...
import io.analytica.spies.impl.javassist.matcher.CompositeMatcher;
import io.analytica.spies.impl.javassist.matcher.CtBehaviorMatcher;
import io.analytica.spies.impl.javassist.matcher.InheritClassMatcher;
import io.analytica.spies.impl.javassist.matcher.Matcher;
import io.analytica.spies.impl.javassist.matcher.RegExpMatcher;
import io.analytica.spies.impl.javassist.matcher.StaticMatcher;
...
/**
 * Impl�mentation de ClassFileTransformer pour instrumenter les m�thodes.
 * Necessite d'�tre plac� dans un jar <b>avec</b> les class de Javassist, car il n'y a pas de gestion de classpath dans le javaagent
 * @author npiedeloup
...
final class AnalyticaSpyTransformer implements ClassFileTransformer {
	...
	AnalyticaSpyTransformer(final String agentArgs) {
		//Assertion.notEmpty(agentArgs); //pas Asssertion pour eviter une d�pendance
		if (agentArgs == null || agentArgs.length() == 0) {
			throw new IllegalArgumentException("Usage : -javaagent:analyticaAgent-1.4.0b.jar=stackJavassistAnalyticaSpy.json");
		}
	...
	public byte[] transform(final ClassLoader loader, final String className, final Class<?> classBeingRedefined, final ProtectionDomain protectionDomain, final byte[] classfileBuffer) throws IllegalClassFormatException {
		if (className == null) {
			//System.err.println("className null " + classBeingRedefined);
			return null;
		} else {
			try {
	 ...
	 * @return si cette classe doit �tre instrument�e
	 */
	...
	private AnalyticaSpyHookPoint lookForHookPoint(final ClassLoader loader, final String adaptedclassName) {
		...
		//Test de d'inclusions par le nom de class (pour ne cr�er la CtClass qu'au besoin)
	...
	private enum MethodType {
		INIT,
		CLINIT,
		METHOD, // pr�sent dans MethodInfo
		ABSTRACT,
		FINAL,
		NATIVE,
		PRIVATE,
		...
		PUBLIC,
		STATIC,
		SYNCHRONIZED // quelques javassist.Modifier
	}
		...
		for (final String attrStr : methodTypesStr) {
			...
			switch (attr) {
				...
				case SYNCHRONIZED:
					...
					break;
				default:
					throw new InvalidParameterException("Type d'attributs non g�r� : " + attrStr);
			}
	...
	private String replaceAlias(final String toReplace, final String className, final String methodName) {
		...
		//updated = updated.replaceAll("\\$collector", "io.analytica.agent.impl.KProcessCollector processCollector = io.analytica.spies.impl.javassist.Container.getProcessCollector();");
		updated = updated.replaceAll("\\$collector", "io.analytica.spies.impl.javassist.Container.getProcessCollector()");
		return updated;
	}
	//
